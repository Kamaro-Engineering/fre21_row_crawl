...
Cone Cone::findCone(const sensor_msgs::LaserScan& scan, double center_angle,
  ...
  int start_index = (center_angle - scan.angle_min) / scan.angle_increment;

  const size_t obstacle_window_size =
      std::max(static_cast<size_t>(1), static_cast<size_t>(std::abs(std::ceil(
                                           obs_angle / scan.angle_increment))));
  ...
  for (int i = start_index; i < scan.ranges.size(); ++i) {
    ...
    bool hit =
        !(distance < 0.1 || std::isnan(distance)) && (distance < ray_length);

    obstacle_window[obstacle_window_index] = hit;
    double points_in_cone = std::count(
        obstacle_window, obstacle_window + obstacle_window_size, true);
